...
一条命令，让模型「看」图像、「听」音频、「看」视频，返回文字或 JSON。

QSense 不是应用，是给上层 skill、agent、脚本调用的最底层感知能力。它只做一件事：**把多模态输入送给模型，拿回文字或结构化结果。** 视频切片、音频分段、批量处理、重试编排、工作流逻辑——这些都不是 qsense 的事，留给上层组合。
...
| 🔌 | **Agent 友好** | 纯文本或 JSON stdout，`[qsense]` stderr 错误，exit 0/1，零副作用 |
...
# ── 结构化输出 ─────────────────────────────────────
qsense --prompt "提取审核结论。" --target screenshot.png --schema review.schema.json --output json

# ── 更高视觉精度 ───────────────────────────────────
qsense --prompt "读取这张密集图表。" --target chart.png --vision-fidelity max
...
选项:
  ...
  --target TEXT         主目标输入（最多 1 个）
  --reference TEXT      对照图、上一版或参考资产
  --context TEXT        辅助上下文文件或媒体
  --spec TEXT           文本或媒体形式的审核规范
  ...
  --output [text|json]  输出模式（默认: text）
  --schema TEXT         可选 JSON Schema，用于校验模型文本输出
...
**QSense 做什么** — 把文件送给模型，返回文字或结构化响应。就这样。

**QSense 不做什么** — 批量遍历、自动重试、工作流编排、对话状态管理、业务专属审核 rubric——全部留给上层 skill 组合。
...
src/qsense/
  ...
  contracts.py      角色化请求契约与提示前缀
  image.py          图像校验、缩放、编码
  audio.py          音频校验、下载、编码
  response.py       结构化响应封装
  schema.py         可选 JSON Schema 校验
  video.py          视频直传与抽帧处理
  models.py         模型注册表加载器
