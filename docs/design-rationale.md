...
QSense 是**原子级多模态感知能力**——不是应用，不是框架，而是最底层的感知 primitive。

它只做一件事：把多模态输入（图像/音频/视频）送给模型，拿回文字结果。

```bash
qsense --prompt "描述" --image photo.png
```
...
- 音频就是下载+编码
- 视频就是编码或抽帧
- 没有流水线，没有状态机，没有 workflow

**复杂逻辑由上层 skill 组合实现**，而不是塞进 qsense 本身。比如：
- "视频内容审核" = 视频下载脚本 + qsense 抽帧分析 + 判断逻辑
- "会议纪要" = 音频切片工具 + qsense 逐段转录 + 汇总 skill
- "OCR 流水线" = 截图工具 + qsense 图像理解 + 结果后处理

QSense 就是这些组合中"把东西给模型看一眼"这一步。
